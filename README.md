...
- `MLB/data/tracking/official_picks_profit_summary_all_time.json`
- `MLB/data/tracking/official_picks_profit_summary_current_regime.json`
...
Those views are published both inside the combined summary artifact and as dedicated scope-specific JSON artifacts:

- `official_picks_profit_summary_all_time.json`
- `official_picks_profit_summary_current_regime.json`

The current-regime rule is intentionally date-based so it can be rebuilt from tracked history alone, including older manual seed rows that may not carry model or policy version metadata. The repo-level reporting contract and rebuild instructions are documented in [MLB/docs/performance_reporting.md](MLB/docs/performance_reporting.md).
